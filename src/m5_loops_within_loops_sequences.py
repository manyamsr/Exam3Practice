...
def run_test_integers():
    ...
    # Test 2:
    expected = [18, 50, 14, 6, 44, 7, -4, -21, -2, -100]
    answer = integers([(18, 50, 'laser'),
                       (14, 6),
                       ['s', 'm', 44, '6', 7],
                       'balloon',
                       [],
    ...
    for k in range(len(sequence_of_sequences)):
        for j in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][j]) is int:
                num_list += [sequence_of_sequences[k][j]]
    return num_list
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        if type(sequence_of_sequences[k]) is str:
            for j in range(len(sequence_of_sequences[k])):
                if type(sequence_of_sequences[k][j]) is str and str.isupper(sequence_of_sequences[k][j]):
                    big_string += str(sequence_of_sequences[k][j])
    return big_string
